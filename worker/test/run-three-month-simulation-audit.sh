#!/usr/bin/env bash
# Build a 90-day local operating history, then audit the resulting correlated
# records before the broader domain/frontend suites are run.
set -euo pipefail
...
setsid npx --no-install wrangler dev --assets ../public --port "${PORT}" > /tmp/pharmaridge-three-month-wrangler.log 2>&1 &
SERVER_PID=$!
cleanup() {
...
WORKER_BASE="${BASE}" node test/tools/simulate-three-months.js
WORKER_BASE="${BASE}" node test/audit.three-month-simulation.js
